...
# 日期：       2021年6月
# 说明：       实验二：提高篇
# 任务1：    主流特征提取网络总结及结构对比分析：见报告；
# 任务2：    预训练模型练习：加载相关预训练模型，并可视化输出网络的结构，能够利用模型的不同层和相关预训练模型的参数值实现图像的特征提取；
# 任务3：    特征可视化练习：自行选择一张图片，将其通过卷积神经网络进行特征提取，对图片经过不同层次网络得到的特征进行可视化，对比分析低层特征和高层特征的特点，要求至少利用两种特征可视化方法。
# --------------------------------------------------------------------------------------------------------------------------------------------------
...
#   ----------任务选择----------
print("任务2：预训练模型练习，请输入'PM'")  # Pre-Trained Model
print("任务3：特征可视化练习，请输入'FV'")  # Feature Visualization
while 1:
    TASK = input('请选择任务：')
...
if TASK == 'PM' or TASK == 'pm':
    ...
    data_transforms = {
        'train': transforms.Compose([
            transforms.RandomResizedCrop(resize_train),
    ...
    if NetName == 'res' or NetName == 'google' or NetName == 'inception':
        ...
        model_ft.classifier._modules['1'] = nn.Conv2d(num_ftrs, 2, tuple_kernel, tuple_stride)
        ...
        model_ft.classifier._modules['1'] = nn.Linear(num_ftrs, 2)
    elif NetName == 'shuffle':
        num_ftrs = model_ft._modules['fc'].in_features
        model_ft._modules['fc'] = nn.Linear(num_ftrs, 2)
    ...
    if NetName == 'res' or NetName == 'google' or NetName == 'inception':
        ...
        model_conv.fc = nn.Linear(num_ftrs, 2)  # res/google/inception net
    elif NetName == 'vgg' or NetName == 'alex':
        num_ftrs = model_conv.classifier._modules['6'].in_features
        model_conv.classifier._modules['6'] = nn.Linear(num_ftrs, 2)    # vgg/alex net
    elif NetName == 'dense':
        num_ftrs = model_conv.classifier.in_features
        model_conv.classifier = nn.Linear(num_ftrs, 2)  # dense net
    elif NetName == 'squeeze':
        ...
        model_conv.classifier._modules['1'] = nn.Conv2d(num_ftrs, 2, tuple_kernel, tuple_stride)    # squeeze net
    elif NetName == 'mobile' or NetName == 'mnas':
        num_ftrs = model_conv.classifier._modules['1'].in_features
        model_conv.classifier._modules['1'] = nn.Linear(num_ftrs, 2)  # mobile/mnas net
    elif NetName == 'shuffle':
        num_ftrs = model_conv._modules['fc'].in_features
        model_conv._modules['fc'] = nn.Linear(num_ftrs, 2)  # shuffle net
    ...
    # So, I only use 'model_conv', the way that Mrs. Li taught in class, to export the ONNX model.
    ...
    if SAVEFLAG == 'ON' or SAVEFLAG == 'on':
        ...
        if NetName == 'res' and NetNum == '18':
            ...
            torch.save(model_conv, './models/resnet18_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/resnet18_TL.onnx')
        elif NetName == 'res' and NetNum == '34':
            torch.save(model_ft, './models/resnet34_ft_TL.pth')
            torch.save(model_conv, './models/resnet34_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/resnet34_TL.onnx')
        elif NetName == 'res' and NetNum == '152':
            torch.save(model_ft, './models/resnet152_ft_TL.pth')
            torch.save(model_conv, './models/resnet152_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/resnet152_TL.onnx')
        elif NetName == 'vgg' and NetNum == '16':
            torch.save(model_ft, './models/vgg16_ft_TL.pth')
            torch.save(model_conv, './models/vgg16_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/vgg16_TL.onnx', operator_export_type=torch.onnx.OperatorExportTypes.ONNX_ATEN_FALLBACK)
        elif NetName == 'vgg' and NetNum == '19':
            torch.save(model_ft, './models/vgg19_ft_TL.pth')
            torch.save(model_conv, './models/vgg19_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/vgg19_TL.onnx', operator_export_type=torch.onnx.OperatorExportTypes.ONNX_ATEN_FALLBACK)
        elif NetName == 'alex':
            torch.save(model_ft, './models/alexnet_ft_TL.pth')
            torch.save(model_conv, './models/alexnet_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/alexnet_TL.onnx', operator_export_type=torch.onnx.OperatorExportTypes.ONNX_ATEN_FALLBACK)
        elif NetName == 'google':
            torch.save(model_ft, './models/googlenet_ft_TL.pth')
            torch.save(model_conv, './models/googlenet_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/googlenet_TL.onnx')
        elif NetName == 'inception':
            torch.save(model_ft, './models/inception_ft_TL.pth')
            torch.save(model_conv, './models/inception_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/inception_TL.onnx')
        elif NetName == 'dense' and NetNum == '121':
            torch.save(model_ft, './models/densenet121_ft_TL.pth')
            torch.save(model_conv, './models/densenet121_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/densenet121_TL.onnx')
        elif NetName == 'dense' and NetNum == '161':
            torch.save(model_ft, './models/densenet161_ft_TL.pth')
            torch.save(model_conv, './models/densenet161_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/densenet161_TL.onnx')
        elif NetName == 'squeeze':
            torch.save(model_ft, './models/squeezenet_ft_TL.pth')
            torch.save(model_conv, './models/squeezenet_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/squeezenet_TL.onnx')
        elif NetName == 'mobile':
            torch.save(model_ft, './models/mobilenet_ft_TL.pth')
            torch.save(model_conv, './models/mobilenet_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/mobilenet_TL.onnx')
        elif NetName == 'shuffle':
            torch.save(model_ft, './models/shufflenet_ft_TL.pth')
            torch.save(model_conv, './models/shufflenet_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/shufflenet_TL.onnx')
        elif NetName == 'mnas':
            torch.save(model_ft, './models/mnasnet_ft_TL.pth')
            torch.save(model_conv, './models/mnasnet_conv_TL.pth')
            torch.onnx.export(model_conv, ff, './nets/mnasnet_TL.onnx')
        print('网络已保存')
    else:
        print('保存已关闭')
...
if TASK == 'FV' or TASK == 'fv':
    ...
    while 1:
        NetName = input('请选择网络：')
        if NetName == 'res':
            my_model = torch.load('./models/resnet34_conv_TL.pth')
            break
        elif NetName == 'vgg':
            my_model = torch.load('./models/vgg16_conv_TL.pth')
            break
        elif NetName == 'alex':
            my_model = torch.load('./models/alexnet_conv_TL.pth')
            break
        elif NetName == 'google':
            ...
            my_model = torch.load('./models/squeezenet_conv_TL.pth')
            break
        elif NetName == 'mobile':
            my_model = torch.load('./models/mobilenet_conv_TL.pth')
            break
        elif NetName == 'shuffle':
            my_model = torch.load('./models/shufflenet_conv_TL.pth')
            break
        elif NetName == 'mnas':
    ...
    preds_result = class_names[preds]
    print('预测结果:', preds_result)
